...
- `dependencies` and `settings` have been deleted (zero consumers verified for both). `shared_widgets` has been dissolved into `design_system`, `packages/features/otp`, and `packages/features/branches`. Do not re-create these packages or depend on them. Settings responsibilities now live in `account_settings` + `organization_settings`.
