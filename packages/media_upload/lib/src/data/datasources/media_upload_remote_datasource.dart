...
abstract interface class MediaUploadRemoteDataSource {
  ...
  /// `DELETE /media/{id}` — removes an already-uploaded file the caller
  /// owns, so a removed-before-submit image doesn't stay orphaned on the
  /// backend.
...
class MediaUploadRemoteDataSourceImpl implements MediaUploadRemoteDataSource {
  ...
  @override
  TaskEither<Failure, Unit> deleteMedia(String id) => TaskEither.tryCatch(
    () async {
      await _client.delete<dynamic>(MediaUploadApiPaths.delete(id));
      return unit;
    },
    (error, _) => ErrorMapper.mapError(error),
  );
