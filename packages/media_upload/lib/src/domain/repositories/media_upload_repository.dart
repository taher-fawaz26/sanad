...
/// /media/upload-single` and `DELETE /media/{id}`. Feature-specific
/// attach/replace operations (e.g. linking a `mediaId` to a service) happen
/// afterwards, outside this package, using [UploadedMedia.mediaId].
abstract interface class MediaUploadRepository {
  ...
  /// Deletes an already-uploaded file by its backend `mediaId`.
  TaskEither<Failure, Unit> deleteMedia(String mediaId);
}
