...
void main() {
  ...
  test('deleteMedia calls DELETE media/{id}', () async {
    when(
      () => client.delete<dynamic>('media/media-1'),
    ).thenAnswer(
      (_) async => Response<dynamic>(
        requestOptions: RequestOptions(path: 'media/media-1'),
      ),
    );

    final result = await dataSource.deleteMedia('media-1').run();

    expect(result.isRight(), isTrue);
    verify(() => client.delete<dynamic>('media/media-1')).called(1);
  });

  test(
    'deleteMedia maps a thrown DioException to a Failure instead of throwing',
    () async {
      when(() => client.delete<dynamic>('media/media-1')).thenThrow(
        DioException(
          requestOptions: RequestOptions(path: 'media/media-1'),
          type: DioExceptionType.connectionError,
        ),
