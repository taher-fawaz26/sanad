...
class MediaRemoteDataSourceImpl implements MediaRemoteDataSource {
  ...
  TaskEither<Failure, ExtractedDocuments> extractDocuments({
    ...
    () async {
      ...
      // Extraction is now a preview: a 200 always carries whatever the
      // extractor could read, including an expired document, unreadable
      // fields, or an Emirates ID front/back mismatch (see
      // `ExtractionResponse.fromJson`). A 400 here is a request-level error
      // (duplicate/unknown media id, wrong media type, or a trade licence
      // sent by an individual) — no longer a document-content rejection — so
      // it maps through `ErrorMapper` like any other failure instead of being
      // intercepted into flagged sections.
