...
///
/// Loads services from the real `GET /services` catalog via
/// [GetServicesListUseCase] (mapped down to the lightweight [ServiceEntity]
/// shape this sheet/its `branches`-package consumers expect), supports
/// multi-select with search, and returns the confirmed selection.
///
/// Previously loaded from a separate `GetServicesUseCase`/`provider/services`
/// endpoint that does not exist on the live backend (confirmed against the
/// OpenAPI spec) — removed, no fallback kept.
  ...
  final selected = await showAppSelectSheet<ServiceEntity>(
    ...
    loadItems: () async {
      ...
      return result.fold(
        (f) => throw f,
        (paged) => [
          for (final record in paged.items)
            ServiceEntity(
              id: record.id,
              name: record.name,
