...
/// limitation, unrelated to this feature), so onboarding widgets read the
...
Future<void> pumpOnboarding(WidgetTester tester, Widget home) async {
...
/// As [pumpOnboarding], but hosts [router] instead of a single `home` widget
/// — for tests that assert navigation between onboarding screens.
Future<void> pumpOnboardingRouter(WidgetTester tester, GoRouter router) async {
  await tester.binding.setSurfaceSize(const Size(1080, 2400));
  tester.view.physicalSize = const Size(1080, 2400);
  tester.view.devicePixelRatio = 3.0;
  addTearDown(() {
  ...
  // OnboardingSplashPage's longer navigation delay — tests that need that
  // call an additional pumpAndSettle(duration) themselves.
  await tester.pumpAndSettle();
}
